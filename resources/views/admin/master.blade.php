                        ...
                        <ul class="dropdown-menu top-dropdown lg-dropdown notification-dropdown">
                            <li>
                                <div class="dropdown-header">
                                    <a class="float-right" href="{{URL::to('admin/notification')}}"><small>View All</small></a> Notification
                                </div>
                                <div class="scrollDiv">
                                    <div class="notification-list">
                                        @if(count($notifications))
                                        @foreach($notifications as $notification)
                                        @if($notification->module=='products')
                                        @php $url=url::to('/admin/product') @endphp
                                        @endif
                                        @if($notification->module=='bookings')
                                        @php $url=url::to('/admin/booking') @endphp
                                        @endif @if($notification->module=='comments')
                                        @php $url=url::to('/admin/reviews') @endphp
                                        @endif @if($notification->module=='disputes')
                                        @php $url=url::to('/admin/dispute') @endphp
                                        @endif
                                        <a class="clearfix" href="{{$url}}">
                                            <span class="notification-icon">
                                                <img alt="" class="rounded-circle" src="{{ asset('admin/assets/img/avtar-2.png') }}" width="50">
                                            </span>
                                            <span class="notification-title">{{$notification->sender->name}}</span>
