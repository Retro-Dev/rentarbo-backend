...
class PaymentController extends Controller
{
    ...
    public function checkAccountStatus()
    {
        ...
        if( !empty($personal_info->id) ){
            $personal_info->id_front = !empty($personal_info->id_front) ? URL::to('storage/'.$personal_info->id_front) : null;
            $personal_info->id_back  = !empty($personal_info->id_back) ? URL::to('storage/'.$personal_info->id_back) : null;
        }
    ...
    {
        $request = $this->__request;
        $param_rule['bank_token']       = 'required';
    ...
    public function earning(){
        $request = $this->__request;
        $record  = UserTransaction::getOwnerEarning($request['user']->id);
        if($record['total']<0){
            $record=array();
        }
        $this->__is_paginate = false;
        ...
        return $this->__sendResponse($record,200,'Data successfully');
    }
   
    public function addBankAccount(Request $request)
    {
        $request = $this->__request;
        $param_rule['country'] = 'required|max:100';
        $param_rule['account_holder_name'] = 'required|max:100';
        $param_rule['routing_number']     = 'required|max:100';
        $param_rule['account_number'] = 'required|max:100';
        ...
        $currentExternalAccount  = UserExternalAccount::getExternalAccountByUserID($request['user']->id);
        $generateBankToken = $this->_gateway->createAccountToken($request['country'],$request['account_holder_name'],$request['routing_number'],$request['account_number']);
        if( $generateBankToken['code'] != 200 ){
            return $this->__sendError('Gateway Error',['message' => $generateBankToken['message']],400);
        }
        //update new external account
        $externalAccount = $this->_gateway->createExternalAccount($request['user']->gateway_connect_id,$generateBankToken['data']['account_token']['id'],true);
        if( $externalAccount['code'] != 200 ){
            return $this->__sendError('Gateway Error',['message' => $externalAccount['message']],400);
        }
        //delete current external account
