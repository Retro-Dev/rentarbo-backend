...
class UserController extends RestController
{
        ...
        switch ($action){
            case 'POST':
                $validator = Validator::make($this->__request->all(), [
                    ...
                    'first_name'          => ['required','min:3','max:50','regex:/^([A-Za-z0-9\s])+$/'],
                    'last_name'          => ['required','min:3','max:50','regex:/^([A-Za-z0-9\s])+$/'],
            ...
            case 'PUT':
                ...
                $validator = Validator::make($this->__request->all(), [
                    ...
                    // 'name'      => ['min:3','max:50','regex:/^([A-Za-z0-9\s])+$/'],
                     'first_name'          => ['nullable','max:50','regex:/^([A-Za-z0-9\s])+$/'],
                    'last_name'          => ['nullable','max:50','regex:/^([A-Za-z0-9\s])+$/'],
                    'image_url' => "max:5000",
        ...
        if( env('VERIFICATION_TYPE') == 'mobile' ) {
            ...
            if( env('SMS_SANDBOX',1) != 1){
                ...
                if( $response['code'] != 200 ){
                    $this->__is_error = true;
                    return $this->__sendError('Mobile Gateway Error',['message' => $response['message'] ],400);
                } else {
    ...
    public function verifyCode()
    {
        ...
        $param_rules['code'] = 'required|in:00000';
    ...
    public function forgotPassword()
    {
        ...
        $this->__is_collection = false;
        $this->__is_paginate   = false;
        return $this->__sendResponse($record,200,__('app.reset_password'));
    }
    ...
    public function socialLogin()
    {
        ...
        $param_rule['first_name']          = 'nullable|max:50';
        $param_rule['last_name']          = 'nullable|max:50';
        ...
        if( $this->__is_error )
            return $response;
        $plaform_id=User::getUserByPlatformID($request->platform_type,$request->platform_id);
        if(!$plaform_id){
            $customerResponse = $this->_gateway->createCustomer([
                'name'  => $request['name'],
                'email' => $request['email'],
        ...
        //update device token
        $api_token = User::updateDeviceToken($request,$user,$request['platform_type'],$request['platform_id']);
        //get updated token record
        $user = User::getUserByApiToken($api_token);
    ...
    {
        $request = $this->__request;
        $param_rule['type'] = 'required|in:debit,credit';

        $response = $this->__validateRequestParams($request->all(),$param_rule);
        if( $this->__is_error )
            return $response;

        $records = UserTransaction::getPaymentHistory($request['user']->id,$request['type']);

        $this->__apiResource = 'PaymentHistory';

        return  $this->__sendResponse($records,200,'Payment history retrieved successfully');
    }
}
