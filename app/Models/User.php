...
class User extends Authenticatable
{
     ...
     */
    protected $fillable = [
        'user_group_id', 'user_type','first_name','last_name','name', 'username', 'slug', 'email', 'mobile_no', 'password', 'image_url','blur_image','status',
        ...
        'created_at', 'updated_at', 'deleted_at','gateway_customer_id','gateway_connect_id'
    ];
    ...
    public function connect_account(){
        return $this->hasOne(UserConnectAccount::class,'user_id','id')
     ...
     * @return bool
     */
    public static function updateDeviceToken($request,$user, $platform_type = 'custom',$platform_id=null)
    {
    ...
    public static function socialUser($params)
    {
        ...
        if( !isset($user->id) ){
            ...
            $username      = self::generateUniqueUserName($params['first_name'] ." ".$params['last_name']);
            $record_id = self::insertGetId([
                ...
                'gateway_connect_id'  => $params['gateway_connect_id'],
                'created_at'      => Carbon::now(),
            ]);
            $data->id = $record_id;
            ...
            $update_data['updated_at']      = Carbon::now();
                $update_data['name'] = $params['first_name']." ".$params['last_name'];
    ...
    public static function getUsersByIds($userIds,$type){
        return self::select('seting.*','users.*','uat.device_type','uat.device_token')
        ->join('user_api_token AS uat','uat.user_id','=','users.id')
        ->join('notification_setting AS seting','seting.user_id','=','users.id')
        ->where([['seting.meta_key','=',$type],['seting.meta_value','=',1]])
        ->whereIn('users.id',$userIds)
        ->groupBy('uat.device_token')
        ->get();
    }
