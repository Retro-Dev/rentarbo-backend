...
class UserCardHook
{
   ...
   */
    public function hook_query_index(&$query,$request, $slug=NULL) {
        //Your code here
        $query->where('user_cards.user_id',$request['user']->id);
        $query->orderByRaw('user_cards.is_default,created_at DESC');
    }
    ...
    */
    public function hook_before_add($request,&$postdata)
    {
        ...
        $postdata['is_default'] = $this->_model::countRecord($request['user']->id) == 0 ? '1' : '0';
    ...
    */
    ...
    {
        $this->_model::where('user_id',$request['user']->id)
        ->update([
            'is_default' => '0'
        ]);
    }
    ...
    */
    ...
    public function create_cache_signature($request)
    {
        ...
        return 'UserCardHook' . md5(implode('',$cache_params));
    }
}
